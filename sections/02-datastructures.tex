...
\subsection{树状数组求逆序对 (Inversion Count by Fenwick Tree)}

\textbf{用途：}
求序列中满足 $i < j$ 且 $a_i > a_j$ 的二元组 $(i, j)$ 个数，即逆序对数量。从右往左扫描序列，树状数组维护「已出现过的值」的个数，对每个元素累加右侧比它小的元素个数。
...
\begin{itemize}
  ...
  \item \textbf{溢出提醒：} 逆序对最多 $\dfrac{n(n-1)}{2}$，须用 \code{long long} 计数。
\end{itemize}

\textbf{时间复杂度：} 离散化与扫描均为 $O(n \log n)$，空间复杂度 $O(n)$。

\lstinputlisting[language=C++]{codes/02-datastructures-12.cpp}

\subsection{归并排序求逆序对 (Inversion Count by Merge Sort)}

\textbf{用途：}
同样求逆序对数量。借助归并排序的分治结构，在合并两个有序段的同时统计跨越左右的逆序对，无需离散化。
...
\lstinputlisting[language=C++]{codes/02-datastructures-11.cpp}
