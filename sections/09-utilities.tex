...
\lstinputlisting[language=bash]{codes/09-utilities-01.bat}

\textbf{使用方法}
\lstinputlisting[language=bash]{codes/09-utilities-02.bat}
...
\lstinputlisting[language=C++]{codes/09-utilities-06.cpp}

\subsection{随机数生成器}

生成对拍所需的\textbf{基础随机数据}。C++11 起推荐使用 \code{mt19937}（梅森旋转引擎），比 \code{rand()} 周期长、分布均匀，且支持任意范围的均匀整数分布。

\lstinputlisting[language=C++]{codes/09-utilities-03.cpp}
...
\lstinputlisting[language=Python]{codes/09-utilities-09.py}

\subsection{单文件对拍}

数据准备好后，用对拍验证正确性：把待测程序写成下面的 \code{Solution} 骨架，核心逻辑集中在 \code{solve()} 中，便于与暴力程序批量比对输出。

\lstinputlisting[language=C++]{codes/09-utilities-07.cpp}
...
\lstinputlisting[language=Python]{codes/09-utilities-08.py}
